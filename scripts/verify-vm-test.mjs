...
const report={version:version.version,profile:version.releaseId,physicalAcceptance:false,priorHardwareReport:'ABI1 NES SMB launches but severe slowdown and visible line-block drawing; current ABI2 memory profile awaits hardware testing',hostStackBudgetBytes:stack,hostRam2StaticBytes:0,hostHeapBytes:symbol(min,'_heap_end')-symbol(min,'_heap_start'),module:JSON.parse(fs.readFileSync(path.join(out,'module.json'))),dosModule:JSON.parse(fs.readFileSync(path.join(out,'dos-module.json'))),artifacts,
  ...
  pending:['physical V1.1.1 GUI boot/update/reset','physical DOS speed, keyboard, Tandy and save tests','physical NES ABI2 memory-profile retest; speed and visible block drawing unresolved','long-run transport stress','old field crash root cause confirmation','future flash/XIP module profiles']};
