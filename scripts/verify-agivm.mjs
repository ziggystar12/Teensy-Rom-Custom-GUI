...
const report={passed:true,physicalAcceptance:false,firmwareVersion:'1.1.1',firmwareUnchanged:true,module:JSON.parse(fs.readFileSync(path.join(out,'module.json'))),artifacts,games,
 checks:['real module idle picker frame ends without cells; keyboard/joystick picker/paging/fire/release/no blank and corrupt-file error','exact C64 main-loop input after an idle picker display using native module packets','generic registry direct AGI routing','RAM1 support and RAM2 state/cache guards','parser/edit, pointer and held input','save/readback and write/flush failures','immutable frame/packet lifecycle','content and actual-6510 keyboard/mouse tests','ARM image bounds/imports/integrity','PAL/NTSC actual CRT reset/start/timeout in VICE'],
