...
export const previewAssetPath = resolve(repositoryRoot, 'docs/ui-preview/assets.js');
...
export function buildDesktopBitmapAssets(
  ...
  for (let code = 0x20; code <= 0x7f; code++) {
    ...
    const rows = mock.glyph[character] || mock.glyph['?'];
...
if (process.argv[1] && resolve(process.argv[1]) === scriptPath) {
  ...
  const previewSource = '// Generated by scripts/generate-desktop-bitmap-assets.mjs.\n'
    + 'export const assets=' + JSON.stringify({font: [...font],
      icons: icons.map(icon => ({id: icon.id, bytes: [...icon.bytes]}))}) + ';\n';
  if (arguments_[0] === '--check') {
    ...
    assert.equal(readFileSync(previewAssetPath, 'utf8').replaceAll('\r\n', '\n'), previewSource,
      'UI preview assets are stale; regenerate the shared bitmap assets');
    console.log('Desktop bitmap assets match the original mock.');
  } else {
    writeFileSync(outputPath, source);
    writeFileSync(previewAssetPath, previewSource);
