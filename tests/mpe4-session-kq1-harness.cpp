...
int main(int argc,char **argv){Run *run=nullptr;std::string error;
  try{require(argc>=2,"usage mpe4-session-kq1 RAW.bin");run=new Run(argv[1]);auto &r=*run;auto &s=r.s();
    ...
    const unsigned fullBeforeIdle=r.fullFrames;
    r.until([&]{return r.session.refineHead;},12);
    require(r.fullFrames==fullBeforeIdle,"idle refinement sends only changed cells");
    const unsigned unchanged=r.changedCells;r.frame();require(r.changedCells==unchanged,"settled refinement does not churn palettes");
    r.key(mpe4::Right);require(!r.session.refineHead,"walking immediately returns to the fast color path");
    r.key(mpe4::Right);r.until([&]{return r.session.refineHead;},12);r.mark("stationary-detail-refines-once-and-cancels-on-movement");
    const unsigned cellsBeforeTyping=r.changedCells,fullBeforeTyping=r.fullFrames;
    r.key('l');require(r.session.parserSplit,"first command character enables sharp bottom strip");
    require(r.fullFrames==fullBeforeTyping&&r.changedCells-cellsBeforeTyping<160,"first character does not resend whole screen");
    for(const char *word="ook";*word;word++)r.key(*word);r.key(mpe4::Enter);
