...
These tests compile and execute the real native C++ engine. They accept game
packages and cartridge fixtures supplied by the caller. The repository
includes a ready-to-play [Black Cauldron demo CRT](../Demo/README.md), with its
own checksums and verification record; the SQ1/KQ1 fixtures and complete game
catalog used below are supplied separately. A CRT is not interchangeable with
the raw game package expected by a runner's `--package` option.

Generated output belongs in an external directory or the ignored `build/`
directory. A C++17 compiler and Node.js are required. On Windows the runners
can use MinGW; pass `--compiler` or set `CXX`/`MPE4_CXX` when it is installed
elsewhere. Use the root `scripts/build-firmware.ps1` workflow for the combined
native firmware and [Build Provenance](../docs/BUILD-PROVENANCE.md) for its
