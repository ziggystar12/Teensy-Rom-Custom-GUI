...
// Earlier releases retain their selected GUI. New builds use the versioned
// reviewed source lock rather than the current HEAD of an enclosing checkout.
const previousGui = {
...
const currentGui = manifest.buildProfile === 'native09' ? json(path.join(root, 'gui/selected-native09/provenance.json')) : null;
const selectedGui = currentGui ? {
  commit: currentGui.sourceCommit,
  snapshotDigest: currentGui.snapshotDigest
...
const extendedCartridge = ['native06', 'native07', 'native08', 'native09'].includes(manifest.buildProfile);
