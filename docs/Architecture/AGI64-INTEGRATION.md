...
and the firmware contains no game. A Power Engine CRT requires the supported
TeensyROM+ Fab0.4 target and its matching MHS firmware even though its packed
layout is EasyFlash or Magic Desk and it retains native recovery code.
...
- Other hardware/firmware/emulators: not guaranteed. VICE does not emulate the
  required firmware service and is outside the supported execution target.
...
complete safety preflight, patch writer, and compositor remain installed as
fail-closed transaction recovery when the matching service declines or cannot
complete work on supported Fab0.4 hardware. Their presence is not a non-Teensy
compatibility promise.

The first game-start loader remains visible. The Teensy+ build does not show
another loader bar for later room or introduction pictures. If the matching
service rejects a request or reports a terminal error, the CRT runs the complete
C64 decoder and renderer behind a black display as fail-closed transaction
recovery. A prefetch miss is also safe; the game decodes synchronously or
recovers natively. Builds made without MHS Power Engine retain their normal
visible loader behavior.
...
## VICE is outside the supported target

VICE emulates EasyFlash and Magic Desk but not the Teensy 4.1, the AGI+
challenge/mailbox, next-picture prefetch, or bus-master DMA. MHS Power Engine
CRTs are not required to boot or play correctly there. Any observed VICE
behavior is diagnostic information only and does not prove compatibility.
