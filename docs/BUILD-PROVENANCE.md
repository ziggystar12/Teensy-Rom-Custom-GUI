...
The current V1.0.5 / native13 build adds shared bitmap controls, consistent dialogs,
and scrolling browser/text views. It retains `/SAVES` and the existing game ABI.
...
[`releases/native13/manifest.json`](../releases/native13/manifest.json).
The selected GUI inputs are locked in `gui/selected-v1.0.5/provenance.json`;
...
native12 releases remain unchanged and can be reproduced from their recorded
source commits.

After validation, `scripts/create-native-release.mjs` verifies the built image
and current source hashes before creating a release directory. V1.0.5 uses
`--build build/native13 --release native13`; rerunning that publication command
...
and release tool derive `MPE_Firmware-V1.0.5.hex` from version `1.0.5`. Both
...
For the next firmware release, increase the final number to `1.0.6`, select a
...
```powershell
node scripts/snapshot-custom-gui.mjs --commit COMMIT --destination gui/selected-v1.0.5 --acme C:\Tools\ACME\acme.exe
```
