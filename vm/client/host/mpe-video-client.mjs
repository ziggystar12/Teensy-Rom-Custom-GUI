...
export function emitVideoClient(e,state,stage){
  ...
  e.label('mpe_video_disable');get(enabled);e.branch(0xf0,'mpe_video_disabled');e.abs(0x20,'wait_fresh_border');e.emit(0x08,0x78);
  ...
  for(const ntsc of [false,true]){
    ...
    for(let i=0;i<(ntsc?29:28);i++)e.emit(0xea);e.emit(0x24,0x03);e.abs(0x4c,0x3000);
  }
  e.label('mpe_video_irq_finish');set(0xd011,0x3b);set(0xd018,0x78);set(0xd019,1);vector('mpe_video_irq');set(0xd012,48);
  e.abs(0xee,state.rasterTicks,'write');e.abs(0x20,'nes_capture_input');e.emit(0x68,0xa8,0x68,0xaa,0x68,0x40);
}
