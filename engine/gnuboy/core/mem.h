...
static byte readb(int a) { return mem_read(a & 0xffff); }
static void writeb(int a, byte b) { mem_write(a & 0xffff,b); }
static int readw(int a) { return readb(a) | (readb(a+1)<<8); }
static void writew(int a,int w) { writeb(a,w);writeb(a+1,w>>8); }
