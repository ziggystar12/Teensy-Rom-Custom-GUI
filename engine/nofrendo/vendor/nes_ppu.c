...
/* PPU access */
#ifdef MPE_NOFRENDO
#define PPU_MEM(x) ppu.page[((x) & 0x3FFF) >> 10][(x) & 1023]
#else
...
#endif // standalone allocation, page mapping and reset
...
#endif // standalone input; MPE routes held controllers and APU
...
uint8 ppu_read(uint32 address)
{
   ...
   case PPU_STAT:
      ...
      if (ppu.strikeflag)
      {
         if ((int32)(nes6502_getcycles(false) - ppu.strike_cycle) >= 0)
...
#endif // MPE supplies indexed palette to firmware
...
{
   // MPE: seven scrolled pixels can be written before visible x=0.
   uint8 *buf = &line[8];
...
#endif // standalone debug viewers
