...
static MPE5_FUNCTION void MPE5VendorDisk(bool write);
...
MPE5_FUNCTION int AAA_AAS(char which_operation)
{
...
#endif
{
...
#ifdef MPE5_NATIVE
	MPE5VendorReset();
	if (!host.addressMap || host.addressMapBytes < mpe5::NativeBackingBytes || !host.bios ||
	...
	for (uint32_t offset = mpe5::NativeTextShadowAddress; offset < mpe5::NativeBackingBytes; offset += 2u)
	{
		mem[offset] = ' ';
		mem[offset + 1u] = 7;
	}
	...
	#ifdef MPE5_NATIVE
	CAST(unsigned)regs16[REG_AX] = MPE5Host.drive.sectors;
	#else
...
static MPE5_FUNCTION bool MPE5VendorRun(uint32_t budget)
{
	...
	for (uint32_t executed = 0; executed < budget &&
		((opcode_stream = mem + 16 * regs16[REG_CS] + reg_ip) != mem); ++executed)
#else
...
#endif
	{
		...
		switch (xlat_opcode_id)
		{
			...
			OPCODE_CHAIN 0: // PUTCHAR_AL
					...
					OPCODE 1: // GET_RTC
						#ifdef MPE5_NATIVE
						memset(mem + SEGREG(REG_ES, REG_BX,), 0, 38);
						break;
						#else
						time(&clock_buf);
						ftime(&ms_clock);
...
#endif
#ifdef MPE5_NATIVE
	if (opcode_stream == mem) MPE5Ready = false;
	return MPE5Ready;
#else
	return 0;
#endif
...
static MPE5_FUNCTION bool MPE5VendorKeyboardPoll()
{
	...
	mem[0x4A6] = key.ascii ? key.ascii : key.scan;
	pc_interrupt(7);
	return true;
}
...
static MPE5_FUNCTION void MPE5ConsoleRefresh()
{
	for (uint16_t row = 0; row < mpe5::CgaTextRows; ++row)
		memcpy(mem + mpe5::NativeTextViewportAddress + 2u * mpe5::CgaTextColumns * row,
			mem + mpe5::NativeTextShadowAddress + 2u * mpe5::NativeTextColumns * row,
			2u * mpe5::CgaTextColumns);
}

static MPE5_FUNCTION void MPE5ConsoleErase(uint16_t first, uint16_t end)
{
	uint8_t *shadow = mem + mpe5::NativeTextShadowAddress;
...
static MPE5_FUNCTION void MPE5VendorPutChar(uint8_t character)
{
	...
	uint8_t *screen = mem + mpe5::NativeTextViewportAddress;
	uint8_t *shadow = mem + mpe5::NativeTextShadowAddress;
...
static MPE5_FUNCTION void MPE5VendorDisk(bool write)
{
	uint32_t length = regs16[REG_AX];
	uint32_t lba = regs16[REG_BP];
	uint8_t *target = mem + SEGREG(REG_ES, REG_BX,);
	...
	for (uint32_t offset = 0; offset < length; offset += 512u)
		if (!MPE5Host.drive.readSector(MPE5Host.drive.context, lba + offset / 512u,
			target + offset)) { regs8[REG_AL] = 1; return; }
	regs8[REG_AL] = 0;
}
