...
static void codeAccess(bool loading){
    ...
    SCB_MPU_RBAR=VM_CODE_BASE|SCB_MPU_RBAR_VALID|11;
    SCB_MPU_RASR=SCB_MPU_RASR_TEX(1)|SCB_MPU_RASR_AP(loading?3:7)|
        (loading?SCB_MPU_RASR_XN:0)|SCB_MPU_RASR_SIZE(16)|SCB_MPU_RASR_ENABLE;
...
static uint32_t openFile(const char *path,VmFileInfo *info){
    ...
    for(unsigned i=0;i<8;i++)if(!files[i]){files[i]=SD.sdfs.open(path,O_RDONLY);if(!files[i])return 0;
...
static int32_t readFile(uint32_t h,uint32_t offset,void *p,uint32_t n){
    if(!h||h>8||!files[h-1]||!files[h-1].seekSet(offset))return -1;return files[h-1].read(p,n);
}
static int32_t nextFile(uint32_t h,VmFileInfo *info){
    if(!h||h>8||!files[h-1]||!files[h-1].isDirectory())return -1;
...
static void closeFile(uint32_t h){if(h&&h<=8)files[h-1].close();}
static bool loadModule(){
    ...
    auto code=(uint8_t *)VM_CODE_BASE;auto data=(uint8_t *)VM_RAM_BASE;
    codeAccess(true);
    ...
    host={VM_ABI,sizeof(VmHost),VM_SERVICES,data+used,VM_RAM_BYTES-used,launch.root,launch.content,timeNow,openFile,readFile,nextFile,closeFile};
    ...
    if(p<VM_CODE_BASE||p>VM_CODE_LIMIT-sizeof(VmModule)||module->abi!=VM_ABI||module->bytes!=sizeof(VmModule)||
       !codePointer((uintptr_t)module->input)||!codePointer((uintptr_t)module->pump)||!codePointer((uintptr_t)module->packet)||!codePointer((uintptr_t)module->ack)){failure=0x14;module=nullptr;return false;}
    return true;
}
...
void VMHostPoll(){
    ...
    module->pump();
    if(pending){if(EZFlashRAM[0xf6]!=sequence)return;module->ack();pending=false;}
