...
const options={};
for(let i=2;i<process.argv.length;i+=2){const k=process.argv[i],v=process.argv[i+1];if(!['--boot-bank','--output','--manifest'].includes(k)||!v)throw new Error(`Unknown/incomplete option ${k}`);options[k]=path.resolve(v);}
