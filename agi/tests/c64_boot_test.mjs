#!/usr/bin/env node
// Run the actual AGI CRT from C64 reset in headless VICE. This validates the
// loader and memory mapping with no Teensy service; its expected endpoint is
// a stable, bounded WAIT timeout, not a AGI prompt or physical bus proof.
...
import {buildMpe3TitleTerminal,MPE3_TITLE_TERMINAL_STATE as stateAddress} from '../../vm/client/host/mpe3-title-terminal.mjs';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");

const options = {
  crt: path.join(root, "build/agivm/SD/AGIVM.crt"),
  manifest: path.join(root, "build/agivm/client.json"),
  out: path.join(root, "build/agivm/c64-boot"),
  standard: "ntsc",
...
const terminal = buildMpe3TitleTerminal({
  gameplay: true, enable1351Mouse: true,
  publishVideoTiming: manifest.publishVideoTiming,
...
assert.deepEqual(bank0.subarray(0xfd, 0xfd + payload.length), payload,
  "CRT boot payload differs from the generated AGI terminal");
...
console.log(`AGI C64 boot passed (${options.standard}): reset -> RAM terminal -> M3TP START -> stable WAIT timeout. ${file("result.json")}`);
