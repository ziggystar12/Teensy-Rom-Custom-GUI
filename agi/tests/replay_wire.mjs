...
const [wirePath,terminalPath,outputPath,noBlankPath]=process.argv.slice(2);
...
const noBlankFrames=new Set(noBlankPath?JSON.parse(fs.readFileSync(noBlankPath)):[]),checkedNoBlank=new Set();
...
assert.deepEqual(checkedNoBlank,noBlankFrames,'every requested dialog frame must be replayed');
...
const result={passed:true,exactNativeWire:true,real6510Presenter:true,packets:packets.length,sidFrames,gameFrames,
  spritePackets,spriteFrames,noBlankFrames:checkedNoBlank.size,
...
fs.writeFileSync(outputPath,JSON.stringify(result,null,2)+'\n');console.log(`PASS AGI C64 wire: ${packets.length} packets, ${sidFrames} exact display frames, ${spritePackets} sprite packets, ${checkedNoBlank.size} dialog frames without display blanking; no physical timing claim`);
