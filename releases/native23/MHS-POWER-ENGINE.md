...
the reported Boulder scrolling issue and checks for the current revision.

V1.0.15 retains the corrected GUI firmware updater and quiet packet recovery.
...
- `MPE_Firmware-V1.0.15.hex`: matching MHS Power Engine firmware.
...
V1.0.15 uses the verified 47-patch combined build. Its manifest records all
...
1. Power off the C64/128, attach TeensyROM+, insert the storage containing the
   kit, and power on.
2. In the TeensyROM menu, open SD or USB and select `MPE_Firmware-V1.0.15.hex`.
...
5. After reboot, open TEENSY > About MPE Firmware and confirm V1.0.15.
...
After installing V1.0.15, copy a newer `MPE_Firmware-Vx.y.z.hex` to the root of
...
than V1.0.15. The filename must have three numeric components without leading
...
The root name for this release is exactly `MPE_Firmware-V1.0.15.hex`. V1.0.9
...
V1.0.15 fingerprints the file without separate SD status probes during the
...
V1.0.15 uses the internal release id `native23`; the release manifest records
...
V1.0.15 writes `/SAVES/MPE4-XXXXXXXX.sav` on the Teensy SD card, creating
